...
模型 ID、effort 与定价复核：2026-09-23。Codex 数据同时由当前工具 schema 和本机 `models_cache.json` 核实。使用时以当前宿主允许的集合为准。

## Codex

| 用户名称 | 精确模型 ID | Codex effort | 默认 |
| --- | --- | --- | --- |
| 6-sol | `gpt-6-sol` | `low`, `medium`, `high`, `xhigh`, `max`, `ultra` | `medium` |
| 6-luna | `gpt-6-luna` | `low`, `medium`, `high`, `xhigh`, `max` | `medium` |
...
| --- | --- | --- | --- | --- | --- |
| `gpt-6-luna` | $0.10 | $0.01 | $0.50 | 1× | 成本敏感、高吞吐、窄范围可重复的工作 |
| `gpt-6-sol` | $2 | $0.20 | $10 | 20× | 需要规划、工具使用、验证与跟进的多步专业工作 |
| `gpt-6-astra` | $10 | $1 | $50 | 100× | 最难的端到端工作、新颖问题、长程自主 |

Codex 子代理文档对 effort 的说明：`low` 用于直接且看重速度的任务；`medium` 是多数代理的平衡默认；`high` 用于追踪复杂逻辑、检查假设、处理边界；`xhigh` 与 `max` 用于特别苛刻的推理；`ultra` 是最深档并带自动委派。未指定时子代理继承父设置或模型默认值。
...
来源：[Codex 子代理与配置](https://learn.chatgpt.com/docs/agent-configuration/subagents)（模型与 effort 选择指引）、[Sol API](https://developers.openai.com/api/docs/models/gpt-6-sol)、[Luna API](https://developers.openai.com/api/docs/models/gpt-6-luna)、[Astra API](https://developers.openai.com/api/docs/models/gpt-6-astra)。当前运行时工具 schema 是模型覆盖和 fork 参数的直接依据。

## Claude Code

| 用户名称 | 精确模型 ID | Claude Code effort | 默认 |
| --- | --- | --- | --- |
| opus-5.5 | `claude-opus-5-5` | `low`, `medium`, `high`, `xhigh`, `max` | `medium` |
...
精确 ID 避免 `opus`、`sonnet`、`fable` 家族别名随版本变动。Opus 4.6 仅在 `task.kind` 为 `writing` 且 `writing_instructions` 明确时进入候选集合，不能误配 `xhigh`。Opus 5.5 要求 Claude Code 2.1.280 或更新版本；本机已核对 2.1.280。`adaptive` 是 thinking 模式；`ultracode` 是 Claude Code 编排设置；二者均不是这里的 effort 值。
...
| `claude-opus-5-5` | $4 | $20 | 2× | 复杂代理式编码的默认选择；跨模块改动、并发与状态类 bug 定位、大 diff 正确性审查、带兼容约束的设计 |
| `claude-opus-4-6` | $5 | $25 | 2.5× | 写作专用候选；擅长遵循明确的目标、读者、语气、结构与约束 |
| `claude-fable-5-1` | $10 | $50 | 5× | 长程自主、新颖或欠定义问题、竞争假设的裁决、错误代价高且难验证的任务；订阅计划可能单独计量额度 |

Claude 侧 effort：`high` 是 API 默认；`xhigh` 是这一代模型（Sonnet 5、Opus 5.5、Fable 5.1）上多数编码与代理任务的推荐档；`max` 只在 xhigh 可测量地留有余量时使用；`medium` 是质量守得住时的降本档；`low` 给简单明确的子任务。同名 effort 在不同模型上不等价。Opus 4.6 不能配 `xhigh`；不能凭版本号给它虚构成本优势或强制分配固定档位。
